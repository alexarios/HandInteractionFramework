...
public class OVRSkeleton : MonoBehaviour
{
	...
	public OVRPlugin.Skeleton2 _skeleton = new OVRPlugin.Skeleton2();
	...
	// List of capsule colliders used for grabbing.
	public List<CapsuleCollider> GrabCapsules;

	public bool ColliderDelay;
	...
	private void InitializeCapsules(Boolean grabCollider)
	{
		bool flipX = (_skeletonType == SkeletonType.HandLeft || _skeletonType == SkeletonType.HandRight);
		...
		if (_enablePhysicsCapsules)
		{
			...
			if (_capsules == null || _capsules.Count != _skeleton.NumBoneCapsules)
			{
				...
				Capsules = _capsules.AsReadOnly();
				GrabCapsules = new List<CapsuleCollider>();
			}
			...
			for (int i = 0; i < _capsules.Count; ++i)
			{
				...
				if (grabCollider)
                {
					...
					GrabCapsules.Add(new GameObject(BoneLabelFromBoneId(_skeletonType, bone.Id) + "_GrabCollider").AddComponent<CapsuleCollider>());
					var grabCapsule = GrabCapsules[i];
					grabCapsule.isTrigger = true;
					grabCapsule.radius = capsule.CapsuleCollider.radius + .002f;
					grabCapsule.height = capsule.CapsuleCollider.height;
					grabCapsule.direction = capsule.CapsuleCollider.direction;
					grabCapsule.center = capsule.CapsuleCollider.center;

					GameObject gcGO = grabCapsule.gameObject;
	...
	private void FixedUpdate()
	{
		...
		if (_enablePhysicsCapsules)
		{
			...
			for (int i = 0; i < _capsules.Count; ++i)
			{
				...
				CapsuleCollider grabCapsule = GrabCapsules[i];
				...
				if (data.IsDataValid && data.IsDataHighConfidence)
				{
					...
					// Updates the position of our grabColliders.
					grabCapsule.transform.position = capsule.CapsuleCollider.transform.position;
					grabCapsule.transform.rotation = capsule.CapsuleCollider.transform.rotation;
				}
				else
				{
					if (capsuleGO.activeSelf)
